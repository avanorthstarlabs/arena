import { categorize, BASE_DAMAGE, STAMINA_COST } from "./actions.js";
 ...
 */
export function resolveExchange(p1Action, p2Action, p1State, p2State) {
    ...
    if (p1Action === "taunt")
        p1StaminaChange += TAUNT_STAMINA_REGEN;
    if (p2Action === "taunt")
        p2StaminaChange += TAUNT_STAMINA_REGEN;
    // --- DODGE: avoids everything, deals nothing ---
    if (c1 === "dodge" && c2 === "dodge") {
        ...
        if (BASE_DAMAGE[p1Action] > 0)
            p1Damage = Math.round(BASE_DAMAGE[p1Action] * mult1);
        if (BASE_DAMAGE[p2Action] > 0)
            p2Damage = Math.round(BASE_DAMAGE[p2Action] * mult2);
