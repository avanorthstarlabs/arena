...
export function createRouter(lobby) {
    const router = Router();
    router.post("/arena/register", (req, res) => {
        try {
    ...
    router.post("/arena/challenge", (req, res) => {
        try {
    ...
    router.post("/arena/accept", (req, res) => {
        try {
    ...
    router.post("/arena/action", (req, res) => {
        try {
            ...
            if (state.status === "fight_over") {
                try {
                    resolution = lobby.resolveSideBets(body.fight_id);
                }
                catch { }
            }
    ...
    router.post("/arena/next-round", (req, res) => {
        try {
            const { fight_id } = req.body;
            const fight = lobby.getFight(fight_id);
            if (!fight)
                return res.status(404).json({ ok: false, error: "Fight not found" });
            fight.nextRound();
    ...
    router.post("/arena/side-bet", (req, res) => {
        try {
    ...
    router.get("/arena/side-bets/:fightId", (req, res) => {
        const { bets, pool } = lobby.getSideBets(req.params.fightId);
        res.json({ ok: true, bets, pool });
    });
    router.post("/arena/resolve-bets/:fightId", (req, res) => {
        try {
